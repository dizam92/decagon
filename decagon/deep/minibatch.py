...
class EdgeMinibatchIteratorNewSplit(object):
    """ This minibatch iterator iterates over batches of sampled edges or
    random pairs of co-occuring edges.
    assoc -- numpy array with target edges
    placeholders -- tensorflow placeholders object
    batch_size -- size of the minibatches
    """
    def __init__(self, adj_mats, adj_mats_train, adj_mats_test, adj_mats_valid, feat, edge_types,
                 batch_size=100, val_test_size=0.01):
        self.adj_mats = adj_mats
        self.adj_mats_train = adj_mats_train
        self.adj_mats_test = adj_mats_test
        self.adj_mats_valid = adj_mats_valid
        self.feat = feat
        self.edge_types = edge_types
        self.batch_size = batch_size
        self.val_test_size = val_test_size
    ...
    def mask_test_edges(self, edge_type, type_idx):
        ...
        val_edges, _, _ = preprocessing.sparse_to_tuple(self.adj_mats_valid[edge_type][type_idx])
        # val_edge_idx = list(range(val_edges.shape[0]))
        test_edges, _, _ = preprocessing.sparse_to_tuple(self.adj_mats_test[edge_type][type_idx])
        # test_edge_idx = list(range(test_edges.shape[0]))
        train_edges, _, _ = preprocessing.sparse_to_tuple(self.adj_mats_train[edge_type][type_idx])
        ...
        adj_train = sp.csr_matrix(
            (data, (train_edges[:, 0], train_edges[:, 1])),
            shape=self.adj_mats_train[edge_type][type_idx].shape)
        self.adj_train[edge_type][type_idx] = self.preprocess_graph(adj_train)
